...
class product_product(models.Model):
    ...
    def _product_post_set_attributes( self, product_tmpl=None, product=None, meli=None, config=None ):
        ...
        if (not variations_candidates):
            ...
            if ( (("barcode" in product._fields and "product_id" in product._fields)) and config.mercadolibre_post_default_code and product.product_id and product.product_id.barcode):
            ...
            if ( (("sku" in product._fields and product.sku)) and config.mercadolibre_post_default_code):
                ...
                attributes.append(attribute)
                product.meli_attributes = str(attributes)
            if ( (("barcode" in product._fields and not "product_id" in product._fields and product.barcode)) and config.mercadolibre_post_default_code):
                #SKU as attribute is default now
                ...
                if (product_tmpl.meli_pub_principal_variant.id == product.id):
                    ...
                    if ( productjson and "variations" in productjson and len(productjson["variations"]) ):
                        ...
                        for ix in range(len(productjson["variations"]) ):
                            ...
                            #TODO: add SKU
                            var_attributes = var_product._update_sku_attribute( attributes=("attributes" in var_info and var_info["attributes"]), set_sku=config.mercadolibre_post_default_code, var_info=var_info)
                            var = {
                                "id": str(var_info["id"]),
    ...
    def x_match_variation_id( self, meli=None, meli_id=None, meli_id_variation=None, product_sku=None, product_barcode=None ):
        ...
        if ( productjson and "variations" in productjson and len(productjson["variations"]) ):
            ...
            for ix in range(len(productjson["variations"]) ):
                ...
                #_logger.info(productjson["variations"][ix])
                if ( barcode==product_barcode or (not barcode and seller_sku==product_sku) ):# or self._is_product_combination(productjson["variations"][ix])):
                ...
                if ( productjson and "variations" in productjson and len(productjson["variations"]) ):
                    ...
                    for ix in range(len(productjson["variations"]) ):
                        ...
                        if ( (barcode and barcode==product.barcode)
                            or (not barcode and seller_sku==product.default_code)
                            or ( len(productjson["variations"])==1 and seller_sku==product.default_code)
                            or (seller_sku==product.default_code)
                            or self._is_product_combination(productjson["variations"][ix])):
                            ...
                            #_logger.info(var)
                            responsevar = meli.put("/items/"+str(meli_id)+'/variations/'+str( target_meli_id_variation ), var, {'access_token':meli.access_token})
                            _logger.info("x_product_post_stock responsevar:"+str(responsevar))
                            if responsevar:
                                #_logger.info(responsevar.json())
                                rjson = responsevar.json()
                                if rjson:
                                    if "error" in rjson:
                                        _logger.error(rjson)
                                        return rjson
                                    _logger.info("x_product_post_stock res:"+str(rjson))
                        else:
                            barcode_nocoincide = (barcode and barcode==product.barcode)
                            con_barcode_en_ml = (not barcode and seller_sku==product.default_code)
                            no_es_variante_unica_o_sku_no_coincide = ( len(productjson["variations"])==1 and seller_sku==product.default_code )
                            combinacion_no_coincide = self._is_product_combination(productjson["variations"][ix])
                            revisar = (not barcode_nocoincide and "barcode_nocoincide ") or ""
                            revisar+= (not con_barcode_en_ml and "con_barcode_en_ml ") or ""
                            revisar+= (not no_es_variante_unica_o_sku_no_coincide and "no_es_variante_unica_o_sku_no_coincide ") or ""
                            revisar+= (not combinacion_no_coincide and "combinacion_no_coincide") or ""
                            res = { 'error': 'verificar publicacion > '+str(revisar) }
                            _logger.error("x_product_post_stock: "+str(res))
                            return res;
                ...
                if (meli_id_variation):
                    ...
                    if (response):
                        ...
                        if (is_not_meli_id_variation):
                            ...
                            #_logger.error(pjson)
                            fix_meli_id_variation = self.x_match_variation_id(meli=meli, meli_id=meli_id, meli_id_variation=meli_id_variation, product_sku=product.default_code, product_barcode=product.barcode )
