odoo.define('point_of_sale.SelectCashierMixinCreditLimit', function(require) {
  'use strict';
  const SelectCashierMixinCreditLimit = (PosComponent) =>
      class ComponentWithSelectCashierMixinCreditLimit extends PosComponent {
    async askPin(employee) {
      const {confirmed, payload: inputPin} =
    ...
    async selectCashier(price) {
      if (this.env.pos.config.module_pos_hr) {
        const employeesList =
            this.env.pos.employees
                .filter(
                    (employee) => employee.id !== this.env.pos.get_cashier().id)
                .map((employee) => {
                  return {
                    id: employee.id,
                    item: employee,
  ...
  return SelectCashierMixinCreditLimit;
});